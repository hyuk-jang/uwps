...
function log() {
    var traceObj = traceOccurPosition(this),
        occurInfo = "   --> " + traceObj.fileName.substr(traceObj.fileName.lastIndexOf("\\")) + " : " + traceObj.lineNumber;
...
function CLIS() {
    ...
    _.each(arguments, function (arg, index) {
        console.log("pOjbect" + ++index, " --> ", util.inspect(arg, true, 10));
    })
    console.log("-------------  CLIS END  ------------- ", traceObj.fileName.substr(traceObj.fileName.lastIndexOf("\\")) + " : " + traceObj.lineNumber);
}
exports.CLIS = CLIS;
...
function CLI() {
    var util = require("util");
    var traceObj = traceOccurPosition(this),
        occurInfo = "   " + traceObj.fileName.substr(traceObj.fileName.lastIndexOf("\\")) + " : " + traceObj.lineNumber + " ( " + traceObj.functionName + " )";
    console.log("-------------   ", occurInfo, convertDateToText(new Date(), "char", 5, 1));
    var argLength = arguments.length - 1;
    _.each(arguments, function (arg, index) {
        if (index % 2 === 0)
            index === argLength ? console.log(util.inspect(arg, true, 10)) : process.stdout.write(String(arg));
        else
            console.log(" --> ", util.inspect(arg, true, 10));
    })
    console.log("-------------  CLI END  -------------  ", traceObj.fileName.substr(traceObj.fileName.lastIndexOf("\\")) + " : " + traceObj.lineNumber);
}
exports.CLI = CLI;
...
function traceOccurPosition(target) {
    ...
    _.each(__stack, function (stack, index) {
        if (index == 2) {
...
function writeFile(path, message, option, callback) {
    ...
    try {
        fs.writeFile(path, message, { flag: option }, function (err) {
...
function searchDirectory(path, callback) {
    ...
    try {
        fs.readdir(path, function (err, files) {
            if (err) console.error(err)
            if (callback != null) callback(err, files);
...
function logFile(message) {
    ...
    var convertMessage = "\r\n\r\n" + convertDateToText(new Date()) + "\r\n" + message + "\r\n";
    writeFile(path, message, "a");
}
exports.logFile = logFile;

// Error Log
function errorLog(errType, mes, exceptionError) {
    ...
    if (exceptionError == null) {
        var traceObj = traceOccurPosition(this);
        errInfo = "\t" + traceObj.fileName.substr(traceObj.fileName.lastIndexOf("/")) + " : " + traceObj.lineNumber + " : " + traceObj.functionName;
    }
...
// Page Nation Dom 생성
function getPageHtml(page, pageName, pageField, totlaCount, listCount) {
    var returnvalue = "",
        _pagecount = 10,
        firstpage = ((parseInt((page - 1) / _pagecount))) * _pagecount + 1,
        finalpage = parseInt(((totlaCount - 1) / listCount + 1)),
        endpage = (firstpage + _pagecount - 1) > finalpage ? finalpage : firstpage + _pagecount - 1;

    if (firstpage > 1) {
        var template = _.template('<a href="<%= pageName %>?page=<%= firstpage-1 %>&<%= pageField %>"><img src="/images/icon3.jpg" width="17px" height="17px" alt="이전" title="이전" /></a>');
        returnvalue += template({ "pageName": pageName, "firstpage": firstpage, "pageField": pageField });
    }
    else {
    ...
    for (var i = firstpage; i <= endpage; i++) {
        if (i == page) {
            ...
            returnvalue += "<a href='" + pageName + "?page=" + i + "&" + pageField + "'><span>" + i + "</span></a>";
        }
    }


    if (endpage < finalpage) {
        var template = _.template('<a href="<%= pageName %>?page=<%= endpage + 1 %>&<%= pageField %>"><img src="/images/icon4.jpg" width="17px" height="17px"  alt="다음" /></a>');
        returnvalue += template({ "pageName": pageName, "endpage": endpage, "pageField": pageField });
    }
    else {
...
function pageNation(page, totalCount, listCount, pageName, pageField) {

    var returnvalue = "";

    var _page = parseInt(page);
    var _totlaCount = parseInt(totalCount);
    ...
    var firstpage = ((parseInt((_page - 1) / _pagecount))) * _pagecount + 1;
    var endpage = firstpage + _pagecount - 1;
    var finalpage = ((_totlaCount - 1) / _listCount + 1);
    ...
    if (firstpage > 1) {
        returnvalue += '<a href="' + _pageName + '?page=' + (firstpage - 1) + '&' + _pageField + '"><img src="/images/pn_prev.gif" alt="이전" title="이전" /></a>';
    }
    else {
    ...
    for (var i = firstpage ; i <= endpage ; i++) {
        if (i == _page)
            ...
            returnvalue += "<a href='" + _pageName + "?page=" + i + "&" + _pageField + "'><span>" + i + "</span></a>";
    }

    if (endpage < finalpage) {
        returnvalue += '<a href="' + _pageName + '?page=' + (endpage + 1) + '&' + _pageField + '"><img src="/images/pn_next.gif" alt="다음" /></a>';
    }
    else {
