 ...
 FROM
	(
		...
		SELECT 
					...
					END AS interval_wh,
					wdd.avg_temp, wdd.total_interval_solar, wdd.total_interval_inc_solar,
					kd.avg_sky,
					twl.water_level
		FROM
			(
			...
			FROM
				...
				WHERE writedate>= "2018-04-01 00:00:00" and writedate<"2018-09-19 00:00:00"
		...
		LEFT JOIN
			...
			SELECT
						...
						ROUND(AVG(avg_inc_solar), 0) AS avg_inc_solar,
						ROUND(SUM(interval_inc_solar), 1) AS total_interval_inc_solar,
			...
			FROM
				...
				SELECT
							...
							AVG(inclined_solar) AS avg_inc_solar,
							AVG(inclined_solar) AS interval_inc_solar,
							AVG(wd) AS avg_wd,
							AVG(ws) AS avg_ws,
							AVG(uv) AS avg_uv,
		...
		JOIN 
			...
			WHERE applydate>= "2018-04-01 00:00:00" and applydate<"2018-09-19 00:00:00"
