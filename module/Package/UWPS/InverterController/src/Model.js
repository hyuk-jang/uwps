...
class Model {
  ...
  onSystemError(troubleCode, hasOccur, msg) {
    ...
    if (hasOccur && _.isEmpty(findObj)) {
      ...
      BU.appendFile(`./log/inverter/error/system/${BU.convertDateToText(new Date(), '', 2)}.txt`, `이상 발생 id:${this.id}\tcode:${troubleCode}\r\n${msg}`);
      ...
      this.systemErrorList = _.reject(this.systemErrorList, systemError => {
        if(systemError.code === troubleCode){
          BU.appendFile(`./log/inverter/error/system/${BU.convertDateToText(new Date(), '', 2)}.txt`, `이상 해제 id:${this.id}\tcode:${troubleCode}\r\n${msg}`);
