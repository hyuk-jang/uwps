  ...
  discoveryRegularDevice() {
    ...
    if (this.hasReceivedData) {
      ...
      if (this.errorCount === 2) {
        const commandSet = this.generationAutoCommand(this.baseModel.device.DEFAULT.COMMAND.LOOP);
        this.executeCommand(commandSet);
        this.requestTakeAction(this.definedCommanderResponse.NEXT);
      } else if (this.errorCount === 4) {
        // 그래도 정상적인 데이터가 들어오지 않는다면
        const commandSet = this.generationAutoCommand(
    ...
    try {
      ...
      switch (dcEvent.eventName) {
        case this.definedControlEvent.CONNECT:
          cmdWakeUp = this.generationAutoCommand(this.baseModel.device.DEFAULT.COMMAND.WAKEUP);
