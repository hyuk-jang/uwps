...
var getMemberInfo = function(member_seq, callback){
    var sql = " SELECT *, ";
    sql += " IFNULL ( (SELECT name  FROM saltern_info WHERE saltern_info_seq = Member.saltern_info_seq ),'') AS salternName ";
...
var createMember = function({saltern_info_seq, userid, username, address, salt, hashPassword, tel, is_deleted }, callback){
    var updatedate = BU.convertDateToText(new Date());
    var writedate = BU.convertDateToText(new Date());

    var sql = "";
    sql += " INSERT INTO `saltern_integratedserver`.`member` ( `type`, `saltern_info_seq`, `userid`, `username`, `address`, `temppw`, `salt`, `password`, `tel`, `is_deleted`, `writedate`, `updatedate`) ";
    sql += " VALUES (";
    sql += "'" + MRF("1") + "'"
    sql += ", '" + MRF(saltern_info_seq) + "'"
    sql += ", '" + MRF(userid) + "'"
    sql += ", '" + MRF(username) + "'"
    sql += ", '" + MRF(address) + "'"
    sql += ", '" + MRF(salt) + "'"
 ...
 * @param {Number} saltern_info_seq 염전서버 시퀀스
 */
var updateMember = function (member_seq, {saltern_info_seq, userid, username, address, salt, hashPassword, tel, is_deleted}, callback) {
    var updatedate = BU.convertDateToText(new Date());
    var sql = " UPDATE `saltern_integratedserver`.`member` SET ";
    sql += " `saltern_info_seq`='" + MRF(saltern_info_seq) + "'";
...
var getOpenPopMember = function (serverList, callback) {
    ...
    _.each(serverList, function (data, index) {
        if (index == 0) {
            strList += data.saltern_info_seq.toString();
        }
        else {
            strList += "," + data.saltern_info_seq.toString();
        }
    ...
    var sql = " SELECT username,saltern_info_seq FROM Member WHERE member.saltern_info_seq IN (" + strList + ") AND is_deleted = 0 ";
