...
class SerialChecker {
  constructor(checkDevice = {
    ...
    // 연결 시도할 장치들의 정보를 분배
    if (checkDevice.port == '')
      this.waitedDeviceList.push(checkDevice);
    else {
      checkDevice.hasError = false;
      this.connectedDeviceList.push(checkDevice);
    }
  }

  doChecker(callback) {
    this.finalCallback = callback || function (result) {
      console.log(result)
    };
